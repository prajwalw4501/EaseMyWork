...
public interface IEmployees extends JpaRepository<Employees, Long> {
	...
	@Query
	(nativeQuery = true,value = "select e.emp_id,e.first_name,e.last_name,e.experience,e.gender,e.phone_no,l.city,l.state,s.type from employees e inner join location l on(e.emp_id=l.employees_emp_id)inner join services s on(e.emp_id=s.employees_emp_id) where l.city=?1")
